...
const tasksForAdmin = () => {
  ...
  if (task === null) {
    ...
    if (newFlight.cost === Number.isNaN) {
      do {
        alert(`You have to enter a numerical value`);
      } while (newFlight.cost === Number.isNaN);
    }
    ...
    const newFlightsWithoutDeleted = flights.filter(
      (flight) => flight.id !== id
    );
...
const askFlights = (flights) => {
  flights.forEach((flight) => {
    console.table(
      `Flight ${flight.id} is flight from ${flight.from}, ​to: ${
        flight.to
      } costs ${flight.cost} € and ${
        flight.scale === true ? "has scale" : "has no scale"
      }.`
    );
  });
};
...
tasksForUser = () => {
  const price = +prompt(
    "Write the maximum price of the flight you want to buy"
  );
  const priceNum = parseFloat(price);
  const pricedFlights = flights.filter((flight) => flight.cost <= priceNum);
  for (let i = 0; i < pricedFlights.length; i++) {
    console.log(
      `Flight ${pricedFlights[i].id} is from ${pricedFlights[i].from}, ​to: ${
        pricedFlights[i].to
      } costs ${pricedFlights[i].cost} € and ${
        pricedFlights[i].scale === true ? "has scale" : "has no scale"
      }.`
    );
  }
  const boughtFlight = prompt(
    "Which flight do you want to buy? (write the id)"
  );
...
const anotherTask = () => {
  const continueTask = prompt("do you want to do another task?(yes/no)");
  if (task) {
    task = task.toLowerCase();
  }
