const bingo = () => {
  ...
  const askBingoCard = () => {
    const continueGame = confirm(
      `Do you want to play with this card? (Click Accept to continue or Cancel to generate new numbers)`
    );
  ...
  const bingoRandomNumbers = () => {
    ...
    if (
      bomboRandomNumbers.includes(numberOfTheBombo) ||
      numberOfTheBombo === 0
    ) {
    ...
    if (
      confirm(
        `The number ${numberOfTheBombo} has come out. Do you want to continue?`
      ) === true
    ) {
  ...
  const playAgain = () => {
    const askPlayAgain = confirm(
      `Do you want to play again (ACCEPT) or finish the game(CANCEL)?`
    )
      ? bingo()
      : alert(`Thanks for playing!`);
  };
