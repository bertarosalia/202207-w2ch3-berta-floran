...
const number1 = parseFloat(askNumber1);
const askNumber2 = prompt("Introduce la segunda cifra");
const number2 = parseFloat(askNumber2);
...
if (Number.isNaN(number1) || (Number.isNaN(number2) && askNumber2 !== null)) {
  alert("Vuelva a introducir un valor numérico");
} else if (number1 > 0 && askNumber2 === null) {
  const raizCuadrada = parseFloat(Math.sqrt(number1)).toFixed(3);
...
console.log(
  `El resultado de la suma es ${resultSum}, el resultado de la resta es ${resultRest} , el resultado de la multiplicación es ${resultMult}, el resultado de la división es ${resultDiv}.`
);
